...
/**
 * borrowed from psych, props to them
 * i borrowed the options stuff too
 * im sorry
 * i just needed that
 * im 13 yo
 * not the best coder in the entire world
 */
class Alphabet extends FlxSpriteGroup
{
	...
	public static function setDialogueSound(name:String = '')
	{
		...
		soundDialog = AssetPaths.sound(name);
		if(soundDialog == null) soundDialog = AssetPaths.sound('dialogue');
	}
...
class AlphaCharacter extends FlxSprite
{
	...
	{
		super(x, y);
		var tex = AssetPaths.getSparrowAtlas('ui/alphabet');
		frames = tex;
